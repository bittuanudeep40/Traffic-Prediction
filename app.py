...
def predict():
    if request.method == 'POST':
        # Example: assuming you collect form inputs here
        input_data = [float(request.form['feature1']), float(request.form['feature2']), ...]
        prediction = model.predict([input_data])[0]  # Example: returns 0, 1, or 2

        # Map numerical prediction to traffic labels
        label_map = {
            0: 'Normal',
            1: 'Moderate',
            2: 'Heavy'
        ...
        return render_template("prediction.html", statement=traffic_label)
